...
namespace Camera_NET
{
    ...
    public class Camera : IDisposable
    {
        ...
        internal static void DisplayPropertyPage(IBaseFilter dev, IntPtr hwndOwner)
        {
            if (dev == null)
                return;
            ...
            hr = NativeMethodes.OleCreatePropertyFrame(hwndOwner, 0, 0, filterInfo.achName, 1, ref oDevice, caGUID.cElems, caGUID.pElems, 0, 0, IntPtr.Zero);
            DsError.ThrowExceptionForHR(hr);
        ...
        public VideoInput VideoInput
        {
            ...
            {
                if (value == null)
                    throw new ArgumentNullException("VideoInput should not be set to null, use Default instead");
        ...
        public static ResolutionList GetResolutionList(IMoniker moniker)
        {
            ...
            ResolutionList ResolutionsAvailableForRGB = null; //new ResolutionList();
            ...
            try
            {
                ...
                ResolutionsAvailableForRGB = GetResolutionsAvailableForRGB(capFilter);
            }
            finally
            {
                if (filterGraph != null)
            ...
            return ResolutionsAvailableForRGB;
        }
        ...
        public static void DisplayPropertyPage_Device(IMoniker moniker, IntPtr hwndOwner)
        {
            ...
            DisplayPropertyPage(theDevice, hwndOwner);
        ...
        public void DisplayPropertyPage_Crossbar(IntPtr hwndOwner)
        {
            ...
            DisplayPropertyPage((IBaseFilter)DX.Crossbar, hwndOwner);
        ...
        {
            DisplayPropertyPage(DX.CaptureFilter, hwndOwner);
        }
        ...
        public Bitmap SnapshotOutputImage()
        {
            ...
            try
            {
                ...
                if (currentImage != IntPtr.Zero)
                {
                    ...
                    bitmap = new Bitmap(structure.Width, structure.Height, (structure.BitCount / 8) * structure.Width, pixelFormat, new IntPtr(currentImage.ToInt64() + 40));
            ...
            catch (Exception)
            {
                ...
                throw;
                //MessageBox.Show("Failed getting image: " + e.Message);
            }
            finally
            {
        ...
        /// <summary>
        /// Renders DirectShow graph.
        /// </summary>
        private void GraphBuilding_RenderNow()
        {
            int hr = 0;

            IPin pinSourceOutput = null;

            try
            {
                // Render
                pinSourceOutput = DsFindPin.ByDirection(DX.CaptureFilter, PinDirection.Output, 0);
                hr = DX.FilterGraph.Render(pinSourceOutput);
                DsError.ThrowExceptionForHR(hr);
            ...
            {
                SafeReleaseComObject(pinSourceOutput);
                pinSourceOutput = null;
            }
        }
        ...
        private void SetSourceParams()
        {
            ...
            try
            {
                ...
                SetSourceParams(pinSourceCapture);
        ...
        private void SetSourceParams(IPin pinSourceCapture)
        {
            int hr = 0;
            ...
            try
            {
                ...
                if (_Resolution != null && _Resolution.Width > 0)
                {
                    v.BmiHeader.Width = _Resolution.Width;
                }

                // if overriding the Height, set the Height
                if (_Resolution != null && _Resolution.Height > 0)
                {
                    v.BmiHeader.Height = _Resolution.Height;
                }

                // if overriding the bits per pixel
                v.BmiHeader.BitCount = 24;
        ...
        private void AddFilter_Source()
        {
            ...
            _ResolutionList = GetResolutionsAvailableForRGB(DX.CaptureFilter);
        }
        ...
        private void AddFilter_Crossbar()
        {
            ...
            try
            {
                ...
                // add the video input device
                //hr = filterGraph.AddSourceFilterForMoniker(_Moniker, null, "Source Filter", out capFilter);
                //DsError.ThrowExceptionForHR(hr);

                graphBuilder = (ICaptureGraphBuilder2)new CaptureGraphBuilder2();

                // set filter graph to the capture graph builder
                //hr = graphBuilder.SetFiltergraph((IGraphBuilder)filterGraph);
                ...
                //graphBuilder.FindInterface(FindDirection.UpstreamOnly, Guid.Empty, capFilter, typeof(IAMCrossbar).GUID, out crossbarObject);
        ...
        /// <summary>
        /// Gets available resolutions with RGB color system for capture filter.
        /// </summary>
        /// <param name="captureFilter">Capture filter for asking for resolution list</param>
        private static ResolutionList GetResolutionsAvailableForRGB(IBaseFilter captureFilter)
        {
            ...
            try
            {
                ...
                resolution_list = GetResolutionsAvailableForRGB(pRaw);
            }
            catch(Exception)
            {
                throw;
        ...
        /// <summary>
        /// Gets available resolutions with RGB color system for capture pin (PinCategory.Capture).
        /// </summary>
        /// <param name="captureFilter">Capture pin (PinCategory.Capture) for asking for resolution list</param>
        private static ResolutionList GetResolutionsAvailableForRGB(IPin pStill)
        {
            ResolutionList ResolutionsAvailableForRGB = new ResolutionList();

            //ResolutionsAvailableForRGB.Clear();
            ...
            for (int iFormat = 0; iFormat < iCount; iFormat++)
            {
                ...
                ResolutionsAvailableForRGB.AddIfNew(resolution);
            }
